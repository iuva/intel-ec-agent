...
### 3. `dual_process_guardian.py` - 双进程互监控保活
- **功能**：实现双进程互相监控的保活机制
- **核心组件**：
...
### 9. `heartbeat_manager.py` - 心跳管理
- **功能**：心跳检测和连接状态管理
- **作用**：维持与服务器的连接状态

### 10. `host_init.py` - 主机初始化
- **功能**：主机环境初始化和配置
- **作用**：应用启动时的环境准备

### 11. `persistent_storage.py` - 持久化存储
- **功能**：数据持久化存储管理
- **作用**：提供数据长期保存能力

### 12. `process_guardian.py` - 进程守护
- **功能**：单进程守护和监控
- **作用**：确保单个进程的稳定运行

### 13. `wake_lock.py` - 唤醒锁管理
- **功能**：防止系统进入睡眠模式
- **作用**：保持应用运行期间系统唤醒状态
