...
class MessageTool:
    ...
    async def show_message_box_async(self,
                                    message: str,
                                    title: str = "System Prompt",
                                    confirm_show: bool = True,
                                    cancel_show: bool = False,
                                    ...
                                    cancel_timeout: Optional[int] = None) -> Optional[str]:
        """
        Non-blocking async version of show_message_box.
        Runs the blocking HTTP call in a thread pool so the event loop
        continues processing WebSocket ping/pong and heartbeats.
        """
...
async def show_message_box_async(msg: str,
                                 title: str = "System Prompt",
                                 confirm_show: bool = True,
                                 cancel_show: bool = False,
                                 confirm_text: str = "OK",
                                 ...
                                 cancel_timeout: Optional[int] = None) -> Optional[str]:
    """
    Non-blocking async convenience function to show message box.
    Use this in async contexts (e.g., adhoc session monitor) to avoid
    blocking the event loop while waiting for user interaction.
    """
    tool = get_message_tool()
    return await tool.show_message_box_async(
        message=msg,
        title=title,
        confirm_show=confirm_show,
        cancel_show=cancel_show,
